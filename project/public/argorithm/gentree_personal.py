...
Person_csv = open(f'./public/argorithm/{studentid}.csv', 'r', encoding="utf-8-sig")
...
for perCourse in Person_data:
    prefix = perCourse['Mã HP'][:2]
    EDUCATION_PROGRAMME_COURSES.append(perCourse['Mã HP'])
    if prefix in counts:
        counts[prefix] += 1
    else:
        counts[prefix] = 1
max_prefix = max(counts, key=counts.get)
...
# EDUCATION_PROGRAMME_COURSES = ["IT3100E", "IT4785E", "IT3070E", "IT3080E", "IT2030", "IT3160E", "IT4142E", "IT3020E",
#                                "IT2110", "IT2120", "IT3210", "IT3220", "IT4948", "IT3420E", "IT2140E", "IT4110E",
#                                "IT4172E", "IT4593E", "IT3312E", "IT3230E", "IT3170E", "IT4082E", "IT3292E", "IT3290E",
#                                "IT3283E", "IT3280E", "IT4015E", "IT5023E", "IT5024E", "IT4549E", "IT4062E", "IT3323E",
#                                "IT4409E", "IT4542E", "IT3191E", "IT4441E", "IT4210E", "IT4735E", "IT4651E", "IT4125E"]
        ...
        while (len(parent_queue) > 0):
            ...
            if (childrenTopo.__contains__("operator")):
                ...
                switch_name, isDuplicate = RegisterAndRenderNode(dot,
                                                                 str(childrenTopo).__hash__(),
                                                                 Operator2NodeStyle(operator))
...
def RegisterAndRenderNode(dot, courseId, style: NodeStyle):
    ...
    if (style == NodeStyle.Root) or (style == NodeStyle.Caller) or (style == NodeStyle.Dependency):
        if graphNodeId[0:2] == max_prefix:
            dot.attr('node', shape='record', color='#ff000042', style='filled', fontcolor='black', fontsize="14",
                     fontname="Tahoma")
        else:
            dot.attr('node', shape='record', color='gray', style='filled', fontcolor='black', fontsize="10",
                     fontname="Tahoma")
        pass
    # Trường hợp là node của học phần điều kiện
    if (style == NodeStyle.And or style == NodeStyle.Or):
        dot.attr('node', shape='circle', color='gray', style='', fontcolor='darkgreen', fontsize="10",
                 fontname="Tahoma")
        pass
    # -------------------------------------------------------------
    if style == NodeStyle.And:
        ...
        try:
            ...
            dot.node(graphNodeId, label="{" + "{id} | {name}  | {condition} | {credit}".format(
                ...
                credit=myCourse['Thời lượng'] + " / " + str(myCourse['TC học phí']) + "đ / " + str(
                    myCourse['Trọng số']),
            ) + "}",
...
for myCourse in standardizedCourses:
    ...
    dot.attr('node', shape='box', color='white', style='filled',
             fontcolor='black')  # Không hiểu sao phải thiết lập thuộc tính ở đây, nếu không thì node đại diện cho cluster sẽ kông đổi atrribute được
...
# Vẽ đồ thị
try:
    filename = OUTPUT_FOLDER + '/' + studentid
    dot.render(filename + "tmp", view=False, format='png')
    os.system(f"tred {filename}tmp > {filename}")
    os.system(f"dot -Tpng {filename} -o {filename}.png")
    os.system(f"dot -Tsvg {filename} -o {filename}.svg")
    print(filename)
    # dot.render('test', view=False, format='png')
    # dot.render('test', view=False, format='svg')
except:
